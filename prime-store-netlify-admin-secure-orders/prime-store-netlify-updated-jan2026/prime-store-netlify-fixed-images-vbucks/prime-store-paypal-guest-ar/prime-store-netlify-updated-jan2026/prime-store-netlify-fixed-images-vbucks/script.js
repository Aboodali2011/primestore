...
function t(en, ar){
  // Minimal i18n helper: if `ar` is provided, return it when Arabic is selected.
  return (currentLang === 'ar' && typeof ar === 'string') ? ar : en;
}

// Language + currency state
let currentCurrency = (localStorage.getItem('prime_currency') || 'JOD').toUpperCase();
currentLang = (localStorage.getItem('prime_lang') || 'ar').toLowerCase();

const FX = { USD: 1, JOD: 0.709, SAR: 3.75 };
const CUR = {
  USD: { code:'USD', en:'USD', ar:'دولار', symbolEn:'$', symbolAr:'$' },
  JOD: { code:'JOD', en:'JOD', ar:'دينار', symbolEn:'JOD', symbolAr:'د.ا' },
  SAR: { code:'SAR', en:'SAR', ar:'ريال', symbolEn:'SAR', symbolAr:'ر.س' },
};

function setLang(lang){
  currentLang = (lang || currentLang || 'ar').toLowerCase() === 'en' ? 'en' : 'ar';
  localStorage.setItem('prime_lang', currentLang);
  document.documentElement.setAttribute('lang', currentLang);
  document.documentElement.setAttribute('dir', currentLang === 'ar' ? 'rtl' : 'ltr');
  // Update a few common labels if present
  const pmTitle = document.querySelector('.pm-title');
  if (pmTitle) pmTitle.textContent = t('Payment method','طريقة الدفع');
  const coLabel = document.querySelector('.checkout-label');
  if (coLabel) coLabel.textContent = t('Contact info (required)','معلومات التواصل (مطلوب)');
...
function setCurrency(cur){
  const c = String(cur || currentCurrency || 'JOD').toUpperCase();
  currentCurrency = FX[c] ? c : 'JOD';
  localStorage.setItem('prime_currency', currentCurrency);
  try { renderProducts(); } catch(e){}
  try { renderCart(); } catch(e){}
  try { renderCheckoutSummary(); } catch(e){}
}
...
function formatMoney(usdAmount){
  ...
  const rate = FX[currentCurrency] || 1;
  const val = usd * rate;
  const sym = currentLang === 'ar' ? (CUR[currentCurrency]?.symbolAr || currentCurrency) : (CUR[currentCurrency]?.symbolEn || currentCurrency);
  // Arabic: put symbol after number for د.ا / ر.س, English: before for $
  if (currentCurrency === 'USD') {
    return `${sym}${val.toFixed(2)}`;
  }
  return currentLang === 'ar' ? `${val.toFixed(2)} ${sym}` : `${val.toFixed(2)} ${sym}`;
}

function injectLangCurrencyControls(){
  const menu = document.getElementById('navMenu');
  if (!menu || document.getElementById('langSelect')) return;
  ...
  li.innerHTML = `
    <select id="langSelect" class="btn" style="padding:8px 10px;">
      <option value="ar">العربية</option>
      <option value="en">English</option>
    </select>
    <select id="currencySelect" class="btn" style="padding:8px 10px;">
  ...
  `;
  menu.appendChild(li);
  const langSel = li.querySelector('#langSelect');
  const curSel = li.querySelector('#currencySelect');
  if (langSel){
...
function renderCart() {
    ...
    if (discEl) discEl.textContent = formatMoney(d.amount);
    if (discLine) discLine.style.display = d.amount > 0 ? 'flex' : 'none';

    totalEl.textContent = formatMoney(subtotal - d.amount);
}
...
document.getElementById("registerForm")?.addEventListener("submit", (e) => {
  ...
  // Access code removed (email verification is used during checkout)
...
async function submitOrderToNetlify(order) {
  try {
    const body = new URLSearchParams({
      ...
      total_usd: String(order.totalUsd ?? ""),
      total_display: String(order.totalDisplay ?? ""),
...
function buildOrderMessage(order) {
  ...
  if (order.subtotalUsd != null) lines.push(`Subtotal: ${formatMoney(order.subtotalUsd)}`);
  if (order.discountCode && Number(order.discountAmount) > 0) lines.push(`Discount (${order.discountCode}): -${formatMoney(order.discountUsd ?? order.discountAmount)}`);
  lines.push(`Total: ${formatMoney(order.totalUsd ?? order.total)}`);
  lines.push("");
  lines.push("Items:");
  (order.items || []).forEach((it) => {
    lines.push(`- ${it.name} x${it.quantity} (${formatMoney(it.price)})`);
  });
  if (order.notes) {
    lines.push("");
    lines.push("Notes:");
...
function openPayConfirm() {
  ...
  const label = method === "paypal" ? "PayPal" : method === "click" ? "Click" : method === "whatsapp" ? "WhatsApp" : method === "instagram" ? "Instagram" : "Discord";
  const modal = document.getElementById("payConfirmModal");
  const text = document.getElementById("payConfirmText");
  if (text) text.textContent = `You selected ${label}. We will create your order and then continue to ${label}.`;
  if (modal) {
...
window.finalizeOrder = async function(channel, notes="") {
  const auth = (typeof getAuth === 'function') ? getAuth() : { loggedIn: false };
  const profile = (typeof getProfile === 'function') ? (getProfile() || {}) : {};
  // Guest fields (fallback)
  const guestName = (document.getElementById('guestName')?.value || '').trim();
  const guestEmail = (document.getElementById('guestEmail')?.value || '').trim().toLowerCase();
  const name = (profile.name || auth.name || guestName || '').trim();
  const email = (auth.email || profile.email || guestEmail || '').trim().toLowerCase();

  if (!email || !String(email).includes('@')) {
    showNotification(t('Please enter a valid email.','يرجى إدخال بريد إلكتروني صحيح.'));
    return;
  }
  if (!name || name.length < 2) {
    showNotification(t('Please enter your name.','يرجى إدخال الاسم.'));
    return;
  }

  // Email verification required
  if (!window.__emailVerified || window.__emailVerifiedFor !== email) {
    showNotification(t('Please verify your email first.','يرجى تأكيد البريد الإلكتروني أولاً.'));
    return;
  }
  ...
  const order = {
    ...
    contactValue,
    items,
    // Store base amounts in USD
    subtotalUsd: Number(subtotal.toFixed(2)),
    discountCode: (appliedDiscount && appliedDiscount.code && d.amount > 0) ? appliedDiscount.code : '',
    discountUsd: Number(d.amount.toFixed(2)),
    totalUsd: Number(total.toFixed(2)),
    // Display currency
    currency: currentCurrency,
    fxRate: (FX[currentCurrency] || 1),
    subtotalDisplay: Number((subtotal * (FX[currentCurrency] || 1)).toFixed(2)),
    discountDisplay: Number((d.amount * (FX[currentCurrency] || 1)).toFixed(2)),
    totalDisplay: Number((total * (FX[currentCurrency] || 1)).toFixed(2)),
  ...
  if (channel === 'paypal') {
    const link = buildPayPalLink(order);
...
async function requestEmailCode(){
  ...
  if (!email || !email.includes('@')) { showNotification(t('Enter a valid email.','أدخل بريد صحيح.')); return; }
  window.__emailVerified = false;
  window.__emailVerifiedFor = email;
  const btn = document.getElementById('sendEmailCodeBtn');
  if (btn) btn.disabled = true;
  try {
    const res = await fetch('/.netlify/functions/request-email-code', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
    ...
    showNotification(t('Verification code sent. Check your email.','تم إرسال كود التحقق. تفقد بريدك.'));
  } catch (e){
    showNotification(t('Failed to send code.','فشل إرسال الكود.'));
  } finally {
...
async function verifyEmailCode(){
  ...
  if (!email || !email.includes('@')) { showNotification(t('Enter a valid email.','أدخل بريد صحيح.')); return; }
  if (!code) { showNotification(t('Enter the code.','أدخل الكود.')); return; }
  ...
  try {
    ...
    if (st) { st.textContent = t('Email verified ✅','تم التحقق ✅'); st.style.display='block'; }
    showNotification(t('Email verified.','تم التحقق من البريد.'));
  } catch (e){
    window.__emailVerified = false;
    const st = document.getElementById('emailVerifyStatus');
    if (st) { st.textContent = t('Invalid code.','كود غير صحيح.'); st.style.display='block'; }
    showNotification(t('Invalid code.','كود غير صحيح.'));
  }
}

/* ====== PayPal Link ====== */
const PAYPAL_ME = 'https://www.paypal.com/paypalme/FahedAli983';
function buildPayPalLink(order){
  // Prefer USD (PayPal.me commonly defaults); also allow selected currency.
  const cur = (order.currency || 'USD').toUpperCase();
  const amt = (order.totalDisplay ?? order.totalUsd ?? order.total ?? 0);
  const amount = Number(amt || 0).toFixed(2);
  // Try "amountCURRENCY" style; fallback to amount only.
  return `${PAYPAL_ME}/${encodeURIComponent(amount)}${encodeURIComponent(cur)}`;
}

function initApp(){
  // Initialize language/currency
  setLang(currentLang);
  setCurrency(currentCurrency);
  injectLangCurrencyControls();
