...
function fmt(order, usdVal, dispVal) {
  const cur = String(order.currency || 'USD').toUpperCase();
  if (dispVal !== undefined && dispVal !== null && dispVal !== '') {
    const n = Number(dispVal);
    if (!Number.isNaN(n)) return `${n.toFixed(2)} ${cur}`;
  }
  const u = Number(usdVal);
  if (!Number.isNaN(u)) return `${u.toFixed(2)} USD`;
  return '';
}

function paypalLink(order){
  const base = 'https://www.paypal.com/paypalme/FahedAli983';
  const cur = String(order.currency || 'USD').toUpperCase();
  const amt = (order.totalDisplay ?? order.totalUsd ?? order.total ?? 0);
  const a = Number(amt || 0).toFixed(2);
  return `${base}/${encodeURIComponent(a)}${encodeURIComponent(cur)}`;
}
}
...
function buildText(order) {
  ...
  lines.push('');
  if (order.subtotal) lines.push(`Subtotal: ${fmt(order, order.subtotalUsd ?? order.subtotal, order.subtotalDisplay)}`);
  if (order.discountCode && Number(order.discountAmount) > 0) lines.push(`Discount (${order.discountCode}): -$${order.discountAmount}`);
  lines.push(`Total: ${fmt(order, order.totalUsd ?? order.total, order.totalDisplay)}`);
  lines.push('');
...
function buildHtml(order) {
  ...
  const discountLine = (order.discountCode && Number(order.discountAmount) > 0)
    ? `<div style="display:flex;justify-content:space-between;gap:10px;"><span>Discount (${esc(order.discountCode)}):</span><strong>-${esc(fmt(order, order.discountUsd ?? order.discountAmount, order.discountDisplay))}</strong></div>`
    : '';
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:700px;margin:0 auto;">
    ...
    <div style="margin-top:16px;padding:14px 16px;border:1px solid #eee;border-radius:12px;">
      ${order.subtotal ? `<div style="display:flex;justify-content:space-between;gap:10px;"><span>Subtotal:</span><strong>${esc(fmt(order, order.subtotalUsd ?? order.subtotal, order.subtotalDisplay))}</strong></div>` : ''}
      ${discountLine}
      <div style="display:flex;justify-content:space-between;gap:10px;margin-top:8px;font-size:18px;">
        <span>Total:</span>
        <strong>${esc(fmt(order, order.totalUsd ?? order.total, order.totalDisplay))}</strong>
      </div>
    </div>

    ${order.notes ? `<p style="margin-top:14px;"><strong>Notes:</strong> ${esc(order.notes)}</p>` : ''}

    <p style="margin-top:18px;color:#555;">Pay via PayPal (Visa/Mastercard): <a href="${esc(paypalLink(order))}">${esc(paypalLink(order))}</a></p>
    <p style="margin-top:6px;color:#555;">Pay via PayPal (Visa/Mastercard): ${paypalLink(order)}\n\nWe will contact you shortly to confirm delivery.</p>
  </div>`;
}
